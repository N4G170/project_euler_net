...
//{ TCP MAIN LOOP
void TCPMainLoop(std::unique_ptr<ServerSocketTCP>& tcp_server_socket, std::atomic<bool>& quit, bool& start_server)
{
    if(!start_server)
        return;
    try
...
//{ UDP MAIN LOOP
void UDPMainLoop(std::unique_ptr<ServerSocketUDP>& udp_server_socket, std::atomic<bool>& quit, bool& start_server)
{
    if(!start_server)
        return;
    try
...
void CheckProblemsFutures(std::atomic<bool>& quit, ProblemsResults* results)
{
	while(!quit)
	{
		results->CheckFuture();
...
int main(int argc, char *argv[])
{
    ...
    else
    {
		...
		if(!start_tcp && !start_udp)
		{
			TerminateSDL();
			std::cerr << "No listen protocol was set. Please verify your data/config.json" << std::endl;
			return -3;
		}

		MessageWriter::Instance()->WriteLineToConsole("MESSAGE: INIT");

        std::unique_ptr<ServerSocketTCP> tcp_server_socket;
        std::unique_ptr<ServerSocketUDP> udp_server_socket;
		ProblemsResults results;
        ...
        try
        {
            if(start_tcp)
            {
                ...
                tcp_server_socket.reset(new ServerSocketTCP(config_json["tcp_default_port"].asInt(), config_json["message_buffer_size"].asInt(), config_json["max_clients"].asInt(), &results));
            }
        ...
        try
        {
            if(start_udp)
            {
                // Try to instantiate the server socket
                // Parameters: port number, buffer size (i.e. max message size), max sockets
                udp_server_socket.reset(new ServerSocketUDP(config_json["udp_default_port"].asInt(), config_json["message_buffer_size"].asInt(), &results));
            }
        }
        catch (SocketException& e)
        {
